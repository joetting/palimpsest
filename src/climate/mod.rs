...
impl Biome {
    ...
    pub fn veg_cover(&self) -> f32 {
        match self {
            Biome::TropicalForest => 0.95, Biome::TemperateForest => 0.80,
            Biome::Grassland => 0.55, Biome::Desert => 0.05,
            Biome::Tundra => 0.30, Biome::Alpine => 0.15,
    ...
    pub fn photosynthesis_rate(&self) -> f32 {
        match self {
            Biome::TropicalForest => 1.0, Biome::TemperateForest => 0.65,
            Biome::Grassland => 0.35, Biome::Desert => 0.05,
            Biome::Tundra => 0.15, Biome::Alpine => 0.10,
            Biome::Glacier | Biome::Ocean => 0.0,
        }
    }
...
/// Top-level climate engine that orchestrates all sub-engines.
...
impl ClimateEngine {
    pub fn new(
        ...
        config: ClimateConfig, wind: WindParams, cell_size_m: f32,
    ) -> Self {
        ...
        let mut carbon = CarbonCycleEngine::new(SilicateWeatheringParams {
            erosion_amplification: 0.5,
            base_rate_ppm_yr_km2: 0.0001,
            ..SilicateWeatheringParams::default()
        });
        carbon.set_cell_size_m(cell_size_m);
        let default_col = ColumnClimate {
            temp_c: 15.0, precipitation_m: 0.8, runoff_m: 0.4,
            moisture: 0.6, biome: Biome::TemperateForest,
        };
    ...
    pub fn step_epoch(
        ...
        let fluxes = self.carbon.compute_fluxes(
            elevations, delta_h, self.grid_width, self.grid_height, &self.thermostat.state,
        );
    ...
    pub fn biome_counts(&self, elevations: &[f32], sea_level: f32) -> [(Biome, usize); 8] {
        ...
        [
            (Biome::Ocean, counts[0]), (Biome::TropicalForest, counts[1]),
            (Biome::TemperateForest, counts[2]), (Biome::Grassland, counts[3]),
            (Biome::Desert, counts[4]), (Biome::Tundra, counts[5]),
