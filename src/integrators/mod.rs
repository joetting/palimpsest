...
where F: Fn(&[f64]) -> PdsRates {
    ...
    for i in 0..n {
        ...
        let d_n = if y_n[i] > 1e-30 { rates_n.destruction[i] / y_n[i] } else { 0.0 };
        let d_star = if y_star[i] > 1e-30 { rates_star.destruction[i] / y_star[i] } else { 0.0 };
...
pub fn soil_biogeo_rates(state: &[f64], temperature: f64, precipitation: f64) -> PdsRates {
    let carbon = state[0];
...
pub fn integrate_soil_biogeochemistry(grid: &mut TerrainGrid, dt: f64) {
    let rows = grid.rows;
    let cols = grid.cols;
    for r in 0..rows {
        for c in 0..cols {
            ...
            let mut state = vec![grid.soil_carbon[[r, c]], grid.soil_nitrogen[[r, c]]];
            let rate_fn = |s: &[f64]| -> PdsRates { soil_biogeo_rates(s, temp, precip) };
